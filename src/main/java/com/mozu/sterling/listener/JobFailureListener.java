        ...
        if (!jobExecution.getAllFailureExceptions().isEmpty()) {
            ...
            for (Throwable e : jobExecution.getAllFailureExceptions()) {
                ...
                if (e.getMessage() == null) {
                    msg = String.format(jobName + " Failed. Unexpected Exception Thrown: %s. Please Contact Support.", e.getClass().toString());
                } else {
                    msg = String.format(jobName + " Failed: %s", e.getMessage());  
                };
